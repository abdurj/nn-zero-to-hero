...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay=0.1, learning_rate=3e-4, device='cpu'):
        ...
        print(f"Number of parameters tensors with weight decay: {len(decay_params)} num params: {num_decay_params}")
        print(f"Number of parameters tensors without weight decay: {len(nodecay_params)} num params: {num_nodecay_params}")
