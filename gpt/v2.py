...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        out = torch.cat([head(x) for head in self.heads], dim=-1)  # (B, T, n_heads * head_size (== n_embed))
        out = self.proj(out)
        return out
...
class Block(nn.Module):
    ...
    def __init__(self, n_embed, n_head):
        ...
        self.sa_heads = MultiHeadAttention(n_heads=n_head, head_size=head_size) # self-attention head
    ...
    def forward(self, x):
        ...
        x = x + self.sa_heads(self.ln1(x)) # (B,T,n_embed)
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.blocks = nn.Sequential(
            ...
            Block(n_embed, n_head=4),
            Block(n_embed, n_head=4),
            nn.LayerNorm(n_embed)  # final layer normalization
        )
    ...
    def forward(self, idx, targets=None):
        ...
        logits = self.lm_head(x) # (B,T,n_embed) @ (B, n_embed, vocab_size) --> (B,T,vocab_size)
